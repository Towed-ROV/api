...
def create_waypoint_session(waypoint_session: WaypointSessionCreate, db: Session = Depends(get_db)):
    session = crud.get_waypoint_session(
        db, session_id=waypoint_session.session_id)
    if session:
        raise HTTPException(
            status_code=400, detail=f"Waypointsession already exist")
...
def get_completed_waypoint_sessions(db: Session = Depends(get_db)):
    ...
    if not waypoints_sessions:
        raise HTTPException(
            status_code=404, detail="Waypointsession(s) not found.")
...
def get_multiple_waypoint_sessions(db: Session = Depends(get_db)):
    ...
    if not waypoints_sessions:
        raise HTTPException(
            status_code=404, detail="No waypointsessions in database.")
    return waypoints_sessions
...
def delete_waypoint_session_by_session_id(session_id: str, db: Session = Depends(get_db)):
    response = crud.delete_waypoint_session_by_session_id(
        db, session_id=session_id)
    if response["code"] == 404:
        raise HTTPException(
            status_code=404, detail="WaypointSession not found")
    elif response["code"] == 200:
        return {"message": f"Successfully deleted WaypointSession: {session_id}"}
